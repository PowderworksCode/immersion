...
// strip. Dragging either previews locally (the swatch, the square's hue, the
// hex box all update as you move) and commits exactly ONCE on release, by
// writing the hex into the field's text input and dispatching the `change`
// event the Rust widget already listens for. No new message type, and the drag
// itself never touches the wire.

(() => {
  if (window.__imColor) return;
  window.__imColor = true;
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!swatch) {
      if (pop && !e.target.closest?.(".im-colorpop")) closePop();
      return;
    }
    e.preventDefault();
